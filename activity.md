...
## Task 20 — Fix Hero badge layout shift during cipher animation — COMPLETE
- User-reported bug: the Hero badge (cipher/decode text effect) resized dynamically as scrambled characters changed word-wrap, jumping the layout up/down — especially disruptive on smaller devices.
- Root cause: `.badge` had no fixed width/height (`display: inline-flex`, shrink-to-fit); a proportional font means same-length random-character strings render at different pixel widths frame to frame, shifting line-wrap points and therefore height.
- Fix: capped `.badge` at `max-width: 560px; width: 100%` (stable width regardless of content), and locked the text span (`.badge-text`, new class) to a fixed 3-line height via `height: calc(1.5em * 3)` + `-webkit-line-clamp: 3` + `overflow: hidden; text-overflow: ellipsis` — the standard multi-line-clamp technique, giving exactly the "hide overflow with an ellipsis" behavior the user asked for as a safety net if scrambled text would need more lines than reserved. `align-items: flex-start` + a small `margin-top` on the dot keeps it aligned to the first line now that text can span multiple lines.
- Live-verified in browser at both desktop (1280px) and mobile (375px) widths: real text renders at a fixed 89px badge height and is NOT truncated (comfortably fits in 3 lines); an artificial worst-case all-wide-glyph scrambled string of the same length renders at the IDENTICAL 89px height (would naturally need 126px/6 lines, but the line-clamp correctly clips it) — confirming the layout-jump bug is fixed in both directions (no growing, no shrinking).
- Reviewer independently confirmed the CSS diff, the `calc(1.5em*3)=63px` math against the measured values, and that `hero.ts` (cipher logic) was untouched. Flagged one non-blocking cosmetic note: the dot's exact vertical alignment against the first line wasn't independently eyeballed, though the core bug fix is objectively confirmed via measurement.
- Tests/build: PASS (41/41 tests).
